...
function GetRadSpell(a) {
    return $find(a);
}
$telerik.toSpell = function(a) {
    return a;
};
$telerik.findSpell = $find;
Telerik.Web.UI.RadSpell = function(a) {
    Telerik.Web.UI.RadSpell.initializeBase(this, [a]);
...
Telerik.Web.UI.RadSpell.prototype = {
    initialize: function() {
        ...
        if (this._spellCheckButton) {
            $addHandlers(this._spellCheckButton, {
                click: this.startSpellCheck
            }, this);
            if (this._spellCheckButton.tagName && this._spellCheckButton.tagName.toLowerCase() != "input") {
                $addHandlers(this._spellCheckButton, {
                    keypress: this.startSpellCheck
                }, this);
            }
        }
    ...
    checkFinishedHandler: function(b, a) {
        b.set_spellChecked(true);
    },
    checkStartedHandler: function(b, a) {
        b.set_spellChecked(false);
    },
    startSpellCheck: function() {
        var a = this.get_textSource();
        if (a != null) {
            this.spellCheck(a);
        } else {
    ...
    spellCheck: function(c) {
        var a = new Sys.CancelEventArgs();
        this.raiseEvent("clientCheckStarted", a);
        if (!a.get_cancel()) {
            var b = {
                dictionaryLanguage: this.get_selectedLanguage(),
                textSource: c,
                spell: this
            };
    ...
    _createElementSource: function(b) {
        var c = null;
        var a = $find(b);
        if (a && Telerik.Web.UI.RadWebControl.isInstanceOfType(a)) {
            c = $create(Telerik.Web.UI.Spell.RadControlTextSource, {
                element: a
            }, null, null);
        } else {
            c = $create(Telerik.Web.UI.Spell.HtmlElementTextSource, {
                element: $get(b)
            }, null, null);
        }
        return c;
        ...
        if (this._textSource == null) {
            if (null != this.get_controlsToCheck() && this.get_controlsToCheck().length > 0) {
                ...
                for (var b = 0; b < a.length; b++) {
                    c[c.length] = this._createElementSource(a[b]);
                }
                this.set_textSource($create(Telerik.Web.UI.Spell.MultipleHtmlElementsSource, {
                    elements: c
                }, null, null));
            } else {
                if (this.get_controlToCheck()) {
                    this.set_textSource(this._createElementSource(this.get_controlToCheck()));
                } else {
    ...
    set_textSource: function(a) {
        this._textSource = a;
    },
    get_spellChecked: function() {
        return this._spellChecked;
    },
    set_spellChecked: function(b) {
        this._spellChecked = b;
    ...
    set_clientTextSource: function(a) {
        this.set_textSource(null);
    ...
    set_ajaxUrl: function(a) {
        this._ajaxUrl = a;
    },
    get_useClassicDialogs: function() {
        return this.get_dialogOpener().get_useClassicDialogs();
    },
    set_useClassicDialogs: function(a) {
        this.get_dialogOpener().set_useClassicDialogs(a);
    },
    add_load: function(a) {
    ...
    add_clientDialogClosing: function(a) {
        this.get_events().addHandler("clientDialogClosing", a);
    },
    remove_clientDialogClosing: function(a) {
        this.get_events().removeHandler("clientDialogClosing", a);
    },
    add_clientCheckCancelled: function(a) {
        this.get_events().addHandler("clientCheckCancelled", a);
    },
    remove_clientCheckCancelled: function(a) {
        this.get_events().removeHandler("clientCheckCancelled", a);
    },
    add_clientCheckFinished: function(a) {
        this.get_events().addHandler("clientCheckFinished", a);
    },
    remove_clientCheckFinished: function(a) {
        this.get_events().removeHandler("clientCheckFinished", a);
    },
    add_clientCheckStarted: function(a) {
        this.get_events().addHandler("clientCheckStarted", a);
    },
    remove_clientCheckStarted: function(a) {
        this.get_events().removeHandler("clientCheckStarted", a);
    }
};
...
Telerik.Web.UI.Spell.HtmlTextSourceBase = function(a) {
    Telerik.Web.UI.Spell.HtmlTextSourceBase.initializeBase(this);
    this._element = a;
    this._checkTextAsHtml = false;
};
Telerik.Web.UI.Spell.HtmlTextSourceBase.prototype = {
    dispose: function() {
        this._element = null;
        Telerik.Web.UI.Spell.HtmlTextSourceBase.callBaseMethod(this, "dispose");
    },
    get_text: function() {
        return "This method must be overridden in an inherited class!";
    },
    set_text: function(a) {},
    ...
    get_checkTextAsHtml: function() {
        return this._checkTextAsHtml;
    },
    set_checkTextAsHtml: function(a) {
        this._checkTextAsHtml = a;
    }
};
Telerik.Web.UI.Spell.HtmlTextSourceBase.registerClass("Telerik.Web.UI.Spell.HtmlTextSourceBase", Sys.Component);
Telerik.Web.UI.Spell.RadControlTextSource = function(a) {
    Telerik.Web.UI.Spell.RadControlTextSource.initializeBase(this, [a]);
};
Telerik.Web.UI.Spell.RadControlTextSource.prototype = {
    initialize: function() {
        Telerik.Web.UI.Spell.RadControlTextSource.callBaseMethod(this, "initialize");
        var a = this.get_element();
        if (a == null) {
            alert("Could not find target RadControl. Please verify that ControlToCheck/ControlsToCheck points to valid RadControl instances.");
            this.set_element({});
        }
    },
    get_text: function() {
        ...
        if (a.get_html) {
            ...
            if (a.get_text) {
                ...
                if (a.get_value) {
                    ...
                    alert("Cannot access text of RadControl.");
                }
            }
            if (!this.get_checkTextAsHtml()) {
                b = b.replace(/</g, "<rslt/&lt;").replace(/>/g, "<rsgt/>&gt;").replace(/<rslt\/&lt;/g, "<rslt/>&lt;");
            }
        }
        return b;
    },
    set_text: function(b) {
        ...
        if (a.set_html) {
            ...
            if (!this.get_checkTextAsHtml()) {
                b = b.replace(/<rslt\/>&lt;/gi, "<").replace(/<rsgt\/>&gt;/gi, ">");
            }
            if (a.set_text) {
                a.set_text(b);
            } else {
...
Telerik.Web.UI.Spell.HtmlElementTextSource = function(a) {
    Telerik.Web.UI.Spell.HtmlElementTextSource.initializeBase(this, [a]);
};
Telerik.Web.UI.Spell.HtmlElementTextSource.prototype = {
    initialize: function() {
        ...
        if (a == null) {
            ...
            if (a.nodeName == "IFRAME" && a.contentWindow != null) {
                this.set_element(a.contentWindow.document.body);
            }
        }
        a = null;
    },
    ...
    set_text: function(a) {
        ...
        if (this.get_element().value != null) {
            if (!this.get_checkTextAsHtml()) {
                a = a.replace(/<rslt\/>&lt;/gi, "<").replace(/<rsgt\/>&gt;/gi, ">");
            }
            this.get_element().value = a;
        } else {
            if (this.get_element().innerHTML != null) {
                this.get_element().innerHTML = a;
            } else {
                alert("No value or innerHTML attribute. Cannot access text.");
            }
        }
...
Telerik.Web.UI.Spell.MultipleHtmlElementsSource = function(a) {
...
Telerik.Web.UI.Spell.MultipleHtmlElementsSource.prototype = {
    ...
    get_text: function() {
        ...
        for (var a = 0; a < this._elements.length; a++) {
            b[b.length] = this._elements[a].get_text();
        }
        return b.join("<controlSeparator><br/></controlSeparator>");
    },
    set_text: function(b) {
        var c = b.split("<controlSeparator><br/></controlSeparator>");
        for (var a = 0; a < this._elements.length; a++) {
            this._elements[a].set_text(c[a]);
        }
    },
